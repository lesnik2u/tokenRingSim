...
bool UserInterface::isMouseCaptured() const { return ImGui::GetIO().WantCaptureMouse; }

// Configures ImGui visual style
...
void UserInterface::render() {
    ...
    if (showNetworkController)
        renderNetworkController();
    renderNodeInspector();
    renderOverlay();
...
// Draws the stats overlay window
void UserInterface::renderOverlay() {
    ...
    if (ImGui::Begin("Stats Overlay", nullptr,
        ...
        size_t totalNodes = 0;
        for (const std::unique_ptr<Ring> &ring : sim.getRings())
            totalNodes += ring->getNodeCount();
...
void UserInterface::renderNetworkController() {
    ...
    if (sim.getRings().empty()) {
        ImGui::TextDisabled("No rings exist in the simulation.");
        ImGui::Spacing();
        if (ImGui::Button("Create Default Ring", ImVec2(-1, 40))) {
            sim.addRing({(float)GetScreenWidth() / 2.0f, (float)GetScreenHeight() / 2.0f}, 200.0f);
            selectedRingIndex = 0;
        }
        ImGui::End();
        return;
    ...
    if (selectedRingIndex < 0 || selectedRingIndex >= static_cast<int>(sim.getRings().size())) {
        selectedRingIndex = 0;
    }
    Ring &worldRing = *sim.getRings()[selectedRingIndex];
    ...
    if (ImGui::BeginTabBar("ControllerTabs")) {
        ...
        if (ImGui::BeginTabItem("General")) {
            ...
            if (sim.getRings().size() > 1) {
                if (ImGui::BeginCombo("Active World",
                                      std::format("World #{}", worldRing.getRingId()).c_str())) {
                    for (int i = 0; i < static_cast<int>(sim.getRings().size()); ++i) {
                        bool isSelected = (selectedRingIndex == i);
                        if (ImGui::Selectable(
                                std::format("World #{}", sim.getRings()[i]->getRingId()).c_str(),
                                isSelected)) {
                            selectedRingIndex = i;
                        }
                        if (isSelected)
            ...
            if (ImGui::Button("Spawn Nodes", ImVec2(-1, 0))) {
                for (int i = 0; i < spawnAmount; ++i) {
                    ...
                    n.setPosition({(float)GetRandomValue(50, GetScreenWidth() - 50),
        ...
        if (ImGui::BeginTabItem("Physics")) {
            ...
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip(
                    "Maximum nodes allowed in a single emergent ring before it splits.");
            ...
            ImGui::SliderFloat("Gravity (Cluster)", &worldRing.physics.boundaryStrength, 0.0f,
                               10.0f);
        ...
        if (ImGui::BeginTabItem("Inspector")) {
            ...
            if (worldRing.getTopologyVersion() != lastClusterTopologyVersion) {
                cachedClusters.clear();
                for (const std::unique_ptr<Node> &node : worldRing.getNodes()) {
                    int cid = node->getClusterId();
                    if (cid != -1)
                        cachedClusters[cid].push_back(node->getId());
                }
                lastClusterTopologyVersion = worldRing.getTopologyVersion();
            }

            if (cachedClusters.empty()) {
                ImGui::TextDisabled("No rings formed.");
            } else {
                ImGui::BeginChild("ClusterList");
                static char clusterLabelBuf[64];
                for (auto &[cid, members] : cachedClusters) {
                    if (members.size() < 3)
                        continue;

                    std::snprintf(clusterLabelBuf, sizeof(clusterLabelBuf), "Ring #%d (%zu nodes)",
                                  cid, members.size());
                    if (ImGui::Selectable(clusterLabelBuf)) {
                        sim.clearSelection();
                        for (int nid : members)
                            sim.selectNode(nid, true);
                    }
                }
                ImGui::EndChild();
            }
        ...
        if (ImGui::BeginTabItem("Rendering")) {
            ...
            bool showTextLocal = visualizer.getShowText();
            if (ImGui::Checkbox("Show Text Labels", &showTextLocal)) {
                visualizer.setShowText(showTextLocal);
            }
...
// Draws the node details inspector
void UserInterface::renderNodeInspector() {
    const std::vector<int> &selectedNodeIds = sim.getSelectedNodes();
    if (selectedNodeIds.empty())
        return;

    if (selectedNodeIds.size() > 1) {
        ImGui::SetNextWindowPos(ImVec2(ImGui::GetIO().DisplaySize.x - 350.0f, 50.0f),
                                ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize({300, 450}, ImGuiCond_FirstUseEver);

        if (ImGui::Begin("Multi-Node Inspector", nullptr)) {
            ImGui::TextColored(ImVec4(0.0f, 0.8f, 1.0f, 1.0f), "Selection: %zu Nodes",
            ...
            if (ImGui::Button("Delete Selection")) {
                ...
                if (!sim.getRings().empty()) {
                    for (std::unique_ptr<Ring> &ring : sim.getRings()) {
                        for (int id : ids)
                            ring->removeNode(id);
                    }
                }
            ...
            for (int nodeId : selectedNodeIds) {
                ...
                if (node) {
                    ImGui::Text("Node %d: %s (Cluster %d)", nodeId, node->getName().c_str(),
                                node->getClusterId());
                }
            }
            ImGui::EndChild();
    ...
    std::string windowTitle = std::format("{} (ID: {})###NodeInspector_{}", node->getName(),
    ...
    if (ImGui::Begin(windowTitle.c_str(), &open)) {
        ...
        if (!nodeRemoved) {
            ...
            ImGui::TextColored(ImVec4(0.8f, 0.8f, 1.0f, 1.0f),
                               "Stored Data (%zu items):", node->getDataCount());
            ImGui::Separator();
            ...
            if (ImGui::BeginTable("Data", 3,
                ...
                for (const std::unique_ptr<DataItem> &data : node->getStoredData()) {
                    ImGui::TableNextRow();
